...
**새 형식 원고는 이 판정을 하지 않습니다.** `meeting.md`의 `banner:`는 **조직위원회가 만든
대문 그림**을 가리킬 때만 적고, 없으면 사이트의 미팅 기본 배경(`site.yaml`의 `/meetings`)에
미팅 이름을 가운데 얹습니다. 뜻 없는 풍경 사진을 미팅의 얼굴로 세우지 않기 위함입니다.
