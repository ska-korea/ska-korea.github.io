...
이 한 줄이면 참가자 명단과 일정표에서 **소속별·직급별 구성**을 파이 차트로 스스로 뽑습니다.
따로 세지 않으셔도 됩니다.

- **미팅이 끝난 뒤에만 나옵니다.** 미리 보시려면 `::: stats when=always`.
- 항목이 여섯을 넘으면 나머지는 '그 밖'으로 묶입니다 — 조각이 많으면 색이 뭉개져서요.
- 발표 주제는 일정표에 `swg=` 를 단 것만 셉니다(제목을 보고 추측하지 않습니다).
